...
class Node {
    ...
    private Node tNode = null; //3번째 third node
    private Node fNode = null; //4번째 fourth node
    ...
    public void settNode(Node node) { this.tNode = node; }

    public Node getfNode() { return fNode; }

    public void setfNode(Node node) { this.fNode = node; }
}
...
class Tree {
        ...
        if (node != null) {
            if (node.getNodeNumber() == parentNodeNumber) { //부모 번호가 노드의 번호와 일치하면
                if (node.getlNode() == null) { //왼쪽 자식으로 생성
                    ...
                    System.out.println(nodeNumber + "번 노드 생성");
                } else if (node.gettNode() == null) { //3번째 자식으로 생성
                    node.settNode(new Node(++nodeNumber));
                    System.out.println(nodeNumber + "번 노드 생성");
                } else if (node.getfNode() == null) {
                    node.setfNode(new Node(++nodeNumber));
                    System.out.println(nodeNumber + "번 노드 생성");
                }
            } else {
                ...
                if (node.gettNode() != null) traversalNode(node.gettNode(), parentNodeNumber);
                if (node.getfNode() != null) traversalNode(node.getfNode(), parentNodeNumber);
            }
        }
    }
    ...
    final  void selectDeleteNode(Node node, int deleteNodeNum) {
        if (node != null) {
            if (node.getNodeNumber() == deleteNodeNum) {
                ...
                if (node.gettNode() != null) selectDeleteNode(node.gettNode(), deleteNodeNum);
                if (node.getfNode() != null) selectDeleteNode(node.getfNode(), deleteNodeNum);
            }
        }
    }
    ...
    final void deleteChildNode(Node node) {
        if (node != null) {
            ...
            if (node.gettNode() != null) deleteChildNode(node.gettNode());
            if (node.getfNode() != null) deleteChildNode(node.getfNode());
    ...
    final int countLeapNode(Node node) {
        if (node != null && node.getNodeNumber() >= 0) {
            if ((node.getlNode() == null) && (node.getrNode() == null)
                    && (node.gettNode() == null) && (node.getfNode() == null)) {
                ...
                if (node.gettNode() != null) countLeapNode(node.gettNode());
                if (node.getfNode() != null) countLeapNode(node.getfNode());
            }
        }
